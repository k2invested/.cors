#!/usr/bin/env python3
"""discord_bot.py — Discord bridge for the cors step kernel.

This keeps Discord as a thin transport layer over loop.run_turn():
  Discord message -> stable contact_id -> kernel turn -> Discord reply

Behavior defaults:
  - DMs always receive a response
  - Guild messages receive a response unless DISCORD_REQUIRE_MENTION=1
  - Turns are serialized through one lock to avoid trajectory races
"""
...
def should_respond(message: Any, client_user: Any, require_mention: bool,
    ...
    class CorsDiscordClient(discord.Client):
        ...
        async def on_message(self, message):
            ...
            if command_response is not None:
                logging.info("discord transport command user=%s command=%r", contact_id, prompt)
            ...
            state_paths = state_paths_for_contact(contact_id)
