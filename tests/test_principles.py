...
P5_CASES = [
    ...
    ("pre_diff_prompt_says_step_hashes_are_causal_memory", lambda: "step hashes are causal memory" in loop.PRE_DIFF_SYSTEM.lower()),
    ("pre_diff_prompt_says_content_hashes_are_evidence", lambda: "content hashes are evidence" in loop.PRE_DIFF_SYSTEM.lower()),
    ("pre_diff_prompt_distinguishes_step_refs_and_gap_refs", lambda: "step refs are execution provenance" in loop.PRE_DIFF_SYSTEM.lower() and "gap refs are gap-surfacing provenance" in loop.PRE_DIFF_SYSTEM.lower()),
    ("pre_diff_prompt_says_only_committed_resolved_observed_counts_as_existing", lambda: "only committed, resolved, or directly observed state counts as existing" in loop.PRE_DIFF_SYSTEM.lower()),
...
P6_CASES += [
    ...
    ("pre_diff_prompt_distinguishes_semantic_tree_and_described_blob_foundations", lambda: "semantic-tree hashes are embeddable compositional units" in loop.PRE_DIFF_SYSTEM.lower() and "described-blob hashes are foundational executable/data blocks" in loop.PRE_DIFF_SYSTEM.lower()),
]
