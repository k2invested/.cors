...
P5_CASES = [
    ...
    ("pre_diff_prompt_describes_reason_as_structural_judgment", lambda: "`reason_needed` = structural judgment over semantic trees, entities, workflows, tools, vocab routes, and persistence decisions." in loop.PRE_DIFF_SYSTEM),
    ...
    ("pre_diff_prompt_describes_reprogramme_as_profile_maintenance", lambda: "`reprogramme_needed` = write semantic state into entity/admin-style profiles after that judgment is already warranted. It edits semantic profile state only, not delete/remove/unlink/move operations." in loop.PRE_DIFF_SYSTEM),
    ("pre_diff_prompt_describes_clarify_as_user_only_after_context", lambda: "`clarify_needed` = ask for user-only information only after available semantic context is exhausted." in loop.PRE_DIFF_SYSTEM),
