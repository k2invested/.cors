...
P5_CASES = [
    ...
    ("pre_diff_prompt_forbids_same_surface_reresolve", lambda: "do not emit another `hash_resolve_needed` for the same hash, path, package, or content surface" in loop.PRE_DIFF_SYSTEM.lower()),
    ("pre_diff_prompt_prefers_reason_or_clarify_over_same_surface_recursion", lambda: "prefer `reason_needed` and then `clarify_needed` or a mutate gap as appropriate rather than recursively re-resolving the same surface" in loop.PRE_DIFF_SYSTEM.lower()),
...
P6_CASES += [
    ...
    ("resolve_hash_admin_renders_entity_surface", lambda: (lambda reg: (setattr(loop, "_skill_registry", reg), "semantic_tree:skill_package:" in loop.resolve_hash(skill("admin").hash, Trajectory()) and "package: name=admin" in loop.resolve_hash(skill("admin").hash, Trajectory()) and "identity:" in loop.resolve_hash(skill("admin").hash, Trajectory()))[1])(registry())),
    ...
    ("resolve_entity_clinton_injects_identity", lambda: "role: cyber security developer" in loop._resolve_entity([skill("tinchy.stryka").hash], registry(), Trajectory())),
